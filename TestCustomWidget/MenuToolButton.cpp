...
MenuToolButtonItem::MenuToolButtonItem(QWidget *parent)
    : QWidget(parent)
{
    m_margin = 2;
    m_interval = 2;
...
QPixmap MenuToolButtonItem::pixmap() const
{
    return m_pixmap;
}
...
QString MenuToolButtonItem::text() const
{
    return m_text;
}
...
QFont MenuToolButtonItem::font() const
{
    return m_font;
}
...
void MenuToolButtonItem::paintIconOnly(QPainter *painter, const QRect &rect)
{
    if (!m_pixmap.isNull())
    {
        ...
        QPixmap draw_pixmap = m_pixmap.scaled(pixmapWidth, pixmapHeight, Qt::KeepAspectRatio);
        painter->drawPixmap(rect.center() - draw_pixmap.rect().center(), draw_pixmap);
    }
}
...
void MenuToolButtonItem::paintTextBesideIcon(QPainter *painter, const QRect &rect)
{
    if (!m_pixmap.isNull() && !m_text.isEmpty())
    {
        painter->setFont(m_font);
        int textWidth = QFontMetrics(m_font).width(m_text) + m_interval;
        ...
        QPixmap draw_pixmap = m_pixmap.scaled(pixmapWidth, pixmapHeight, Qt::KeepAspectRatio);
        painter->drawPixmap(iconRect.center() - draw_pixmap.rect().center(), draw_pixmap);
        painter->drawText(textRect, Qt::AlignCenter, m_text);
    }
}

void MenuToolButtonItem::paintTextUnderIcon(QPainter *painter, const QRect &rect)
{
    if (!m_pixmap.isNull() && !m_text.isEmpty())
    {
        painter->setFont(m_font);
        int textWidth = QFontMetrics(m_font).width(m_text);
...
void MenuToolButton::setList(const QList<QPixmap> &pixmaps, const QStringList &list)
{
    m_menu->clear();
    m_menu_items.clear();
...
void MenuToolButton::setPopAreaStyle(const MenuToolButton::PopAreaStyle &style)
{
    m_pop_area_style = style;
}

MenuToolButtonItem* MenuToolButton::menuAddAction(const QPixmap &pixmap, const QString &text)
{
...
void MenuToolButton::contextMenuEvent(QContextMenuEvent *event)
{
    showMenu();
}
...
void MenuToolButton::paintEvent(QPaintEvent *event)
{
    ...
    if (m_show_pop_area) {
        ...
        if (m_pop_area_style == PopAreaStyle::kToolButtonCenterRight)
        {
            ...
            painter.drawLine(poprect.left(), rect.top() + 5, poprect.left(), rect.bottom() - 5);
        }
        else if (m_pop_area_style == PopAreaStyle::kToolButtonCenterLeft)
        {
            // 画竖线
            painter.setPen(QPen(Qt::gray, 1)); // 设置画笔颜色和宽度
            painter.drawLine(poprect.right(), rect.top() + 5, poprect.right(), rect.bottom() - 5);
        }
        ...
        if (m_pop_area_arrow_pix.isNull()) {
            ...
            QPixmap draw_pixmap = m_pop_area_arrow_pix.scaled(imageRect.width(), imageRect.height(), Qt::KeepAspectRatio);
            painter.drawPixmap(imageRect.center() - draw_pixmap.rect().center(), draw_pixmap);
        }
...
//        // 画竖线
//        painter.setPen(QPen(Qt::gray, 1)); // 设置画笔颜色和宽度
//        painter.drawLine(sep_line_pos, rect.top() + 5, sep_line_pos, rect.bottom() - 5);
...
//            int W2 = this->rect().width() - W1;
//            int L1 = W2 * 1 / 3;
//            QRect imageRect = this->rect().adjusted(sep_line_pos + L1, 0, -L1, 0);
